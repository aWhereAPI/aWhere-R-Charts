...
#'
#' @description
#' \code{getDays} Generate the starting and ending days for a forecast. 
#'
#' @details
...
#' @param - starting.day: (string) the first day of the forecast. Acceptable values are "today","yesterday", "tomorrow",
#'                        or a specific date with format "YYYY-MM-DD"
#' @param - forecast.days: (vector of numeric value(s)) duration of the forecast(s). For a 7- and 3-day forecast: c(7,3). 
#' @param - ending.day: (optional, string with format "YYYY-MM-DD") if supplied, this will be the last day of the forecast.
#'                      Otherwise, the ending day is calculated by adding the greatest value in forecast.days to the starting date.
#'
#' @return  # days (list of characters) starting day in position 1; ending day in position 2
#'
#' @examples
#' 
#' @export
...
GetDays <- function(starting.day, forecast.days, ending.day=FALSE){
  ...
  # check if the forecast duration is greater than 15 days 
  if (as.Date(day.end) - as.Date(day.start) > 15) {
    print("Forecast duration cannot exceed 15 days.")
  }
...
#'
#' @description
#' \code{getForecastData} pulls forecast data from the aWhere API for the specified days/years.
#'
#' @details
#' pulls forecast data from the aWhere API for the specified days/years.
#'
#' @param - template.place: (data.frame) contains the geographic data for each grid cell
#' @param - days: (list of characters) starting day in position 1; ending day in position 2
#'                each with the format "YYYY-MM-DD"
#' @param - years: (list of numerics) starting year in position 1; ending year in position 2
#                  used to calculate long term values
#' @param - write.file: (optional, boolean with default value FALSE) when true, the forecast data is written to a .csv file 
#' @param - filename.out (optional, character string with default value "forecast") used to name the output file 
#'
#' @return forecast.all: (data.frame) containing all of the n-day forecast data.
#'
#' @examples
#' 
#' @export
...
#' @title GetForecastSummary
#'
#' @description
#' \code{GetForecastSummary} Sum the n-day forecast values at every location.
#'
#' @details
#' Sum the n-day forecast values at every location.
#'
#' @param - forecast.all: (data.frame) containing all of the n-day forecast data.
#' @param - n.day.forecasts (vector of numeric value(s)) duration of the forecast(s).
...
#'
#' @description
#' \code{MapForecast} CMaps the Forecast
#'
#' @details
#' Calls the MakeMap function to map forecasted weather data aggregated by n-days
#' (as specified in the forecasts.n data frame). Current functionality only supports
#' precipitation data, but can easily be modified to map other variables as well.
#'
#' @param - forecasts.n (data frame) contains the columns "n.day" (the number of days
#'                       used to aggregate forecast data), precip.amount.sum (aggregated)
#'                       precipitation amount), and shapewkt (polygon data), the data to be 
#'                       mapped.
#' @param - base.map (ggmap raster) map object on which to overlay the gridded weather data
#' @param - thresholds (data frame) data frame containing the minimum (first row) and maximum 
...
#'
#' @description
#' \code{MakeMap} this function creates a climark map using CLIMARK data 
#'
#' @details
#' this function creates a climark map using CLIMARK data 
#'
#' @param - df Data frame ("df") containing CLIMARK data
#' @param - v: Character string specifying the variable to map. Default "pre".
#'             Acceptable values for "v" are: 
#'                  "pre" = precipitation
#'                  "ppet" = P / PET 
#'                  "pltn" = recip vrs. LTN precip
#' @param - base.map terrain map image from Google Maps to overlay the mapped variable
#' @param - map.name: character string to name the map and output accordingly
#' @param - write.file: (optional, boolean with default value FALSE) when true, the
#'                      forecast data is written to a .csv file
#'
#' @return  values input vector, altered to have new minimum/maximum values. 
#'
#' @examples
